...
Therefore, the correct option is **Newton** (Option C).`,
  ...
  'q-13': `In classical mechanics, linear momentum ($p$) is defined as the product of the mass ($m$) of an object and its velocity ($v$):
$$p = mv$$

Momentum is a vector quantity (it has both magnitude and direction) pointing in the same direction as the velocity.
...
Therefore, the correct option is **Methane** (Option B).`,
  ...
  'q-17': `The functional group containing an oxygen atom bonded to a hydrogen atom ($\\text{-OH}$) is called the **hydroxyl group** (Option C). Organic compounds containing this group are classified as alcohols.
...
- **Carboxyl**: contains a carboxyl group ($\\text{-COOH}$).`,
  ...
  'q-18': `Organic chemistry is defined as the chemistry of carbon compounds. Therefore, organic compounds must primarily contain the element **Carbon** (Option C). Carbon has 4 valence electrons, allowing it to form stable covalent bonds with itself and other elements, creating a wide variety of molecular structures.`,
};
    ...
    if (apiKey) {
      try {
        ...
        const historyPrompt = chatHistory && chatHistory.length > 0
          ? `Conversation history:\n${chatHistory.map((h: any) => `${h.role === 'user' ? 'Student' : 'Tutor'}: ${h.content}`).join('\n')}\n\n`
          : '';
...
Topic: ${topic || 'General'}
Options:
${options?.map((o: any) => `- Option ${o.id.toUpperCase()}: ${o.text}`).join('\n')}
...
${options?.map((o: any) => `   - **Option ${o.id.toUpperCase()}** (${o.text}): ${o.id === correctOptionId ? 'Correct answer.' : 'Incorrect option.'}`).join('\n')}
