...
import type { User, Folder, Exam, Question, Attempt, Answer, AIFeedback, UserRole } from './types'
...
interface ExamStore {
  ...
  login: (email: string, role: UserRole, fullName?: string, avatarUrl?: string) => Promise<void>
  logout: () => void
  ...
  // Exams
  exams: Exam[]
  addExam: (exam: Omit<Exam, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'questionCount' | 'attemptCount'>) => Promise<Exam>
  updateExam: (id: string, updates: Partial<Exam>) => Promise<void>
  deleteExam: (id: string) => Promise<void>
  publishExam: (id: string, publish: boolean) => Promise<void>
  ...
  // Attempts
  attempts: Attempt[]
  startAttempt: (examId: string) => Attempt
  submitAttempt: (attemptId: string) => Promise<void>
  updateAttemptWarnings: (attemptId: string) => Promise<void>
  getCurrentAttempt: (examId: string) => Attempt | undefined
  
  // Answers
  answers: Answer[]
  saveAnswer: (attemptId: string, questionId: string, optionId: string) => Promise<void>
  getAttemptAnswers: (attemptId: string) => Answer[]
  
  // AI Feedback
  aiFeedback: AIFeedback[]
  generateFeedback: (attemptId: string) => Promise<AIFeedback>
}
    ...
    (set, get) => ({
        ...
        set((state) => {
          const updatedExams = state.exams.map((e) =>
            e.id === questionData.examId
              ? { ...e, questionCount: (e.questionCount || 0) + 1 }
              : e
          )
          return {
        ...
        set((state) => {
          const updatedExams = state.exams.map((e) =>
            e.id === question?.examId
              ? { ...e, questionCount: Math.max(0, (e.questionCount || 0) - 1) }
              : e
          )
          return {
      ...
      startAttempt: (examId) => {
        ...
        fetch('/api/attempts', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id, examId, userId: get().user?.id || 'student-1' }),
          ...
          set((state) => ({
            answers: state.answers.map((a) =>
              a.id === existingAnswer.id
                ? { ...a, selectedOptionId: optionId, updatedAt: new Date() }
                : a
            ),
          }))
        } else {
      ...
      generateFeedback: async (attemptId) => {
        const attempt = get().attempts.find((a) => a.id === attemptId)
        if (!attempt) return {} as AIFeedback
